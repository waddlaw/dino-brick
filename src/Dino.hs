...
data Game = Game
  { -- | dinosaur
    ...
    _highscore :: Score,
    -- | countdown for standing up after ducking
    _duckCountdown :: Int
  }
  deriving (Show)
...
incScore g = case g ^. scoreMod of
  ...
  where
    incAndMod x = (x + 1) `mod` constScoreMod
...
moveDino g =
  let d = g ^. dir
   in case d of
        ...
        Down ->
          if shouldStopDino d g
            then setDinoDir Still g
            else
              ( let gNext = moveDino' (-1) g
                 in if isDinoBottom gNext then setDinoDir Still gNext else gNext
              )
...
deleteBarrier g =
  case viewl $ g ^. barriers of
    EmptyL -> g
    a :< as ->
      let x = getBarrierRightmost a
       in (if x <= 0 then g & barriers .~ as else g)
...
spawnBarrier g =
  ...
  let (DiffMod wm hm (d : ds)) = getDiffMod g
   ...
   in case viewr $ g ^. barriers of
        ...
        _ :> a ->
          let x = getBarrierLeftmost a
           in if (gridWidth - x) > d then setDiffMod newDiffMod (addRandomBarrier g) else g
